...
const { sendMail, appMagicLinkEmail } = require('../common/lib/mailer');
...
const run = async () => {
  ...
  const path = resolve('./common/lib/mailer/magic-link.pug');
  ...
  await Promise.map(apps, async (app) => {
    if (!singleApp || app.id === singleApp) {
      const mail = appMagicLinkEmail(app, renderer);
      await sendMail(mail);
    }
  });
  // const app = apps[0];
